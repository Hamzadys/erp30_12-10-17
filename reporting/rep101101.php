...
//		$salesfolk = get_salesman_name($folk);
	...
	$cols = array(0, 40,   100,  325,   500);

	$headers = array(_('PO.No.'),_('Ord.Date'), _('Description'), _('Qty Ordered'), _('Qty Delivered'));
...
//	$result = db_query($sql, "The customers could not be retrieved");
	...
	while ($myrow = db_fetch($GetRecord))
	{
        if($myrow['reference'] == 'auto')
            continue;
        if ($no_zeros && $myrow['quantity_received'] == 0)
            continue;
        ...
        $rep->TextCol(4, 5, $myrow['quantity_received']);
